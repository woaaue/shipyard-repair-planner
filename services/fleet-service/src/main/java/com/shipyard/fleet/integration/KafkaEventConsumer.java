...
public class KafkaEventConsumer {
    ...
    @KafkaListener(topics = {"repair.schedule.requested"}, groupId = "fleet-service")
    public void handleEvent(String raw, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic) {
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                log.info("Fleet service consumed event {}", root.path("eventType").asText());
                return;
            } catch (Exception ex) {
                lastError = ex;
                log.warn("Fleet service attempt {}/{} failed for topic {}", attempt, maxAttempts, topic, ex);
            }
        }

        deadLetterPublisher.publish(topic, raw, lastError == null ? "unknown error" : lastError.getMessage());
        log.error("Fleet service moved event from topic {} to DLQ after {} attempts", topic, maxAttempts);
    }
}
